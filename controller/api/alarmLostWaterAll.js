...
module.exports.GetAlarmLostWaterAll = async function (req, res) {
  let result = [];
    ...
    for (let site of sites) {
      if (site.LoggerId != null && site.LoggerId != undefined) {
        ...
        if (channel.length > 0) {
          ...
          let time = await DataLogger.find({}).sort({ TimeStamp: -1 }).limit(1);

          if (time.length > 0) {
            if (time != null && time != undefined) {
              if (time[0].TimeStamp != null && time[0].TimeStamp != undefined) {
