...
let urlGetQuantityMonthForcast = `${hostname}/GetQuantityMonthForcast`;
...
let viewQuantityMonthForcast = document.getElementById(
  "viewQuantityMonthForcast"
);

viewQuantityMonthForcast.addEventListener("click", function (e) {
  ...
  if (siteid == null || siteid == undefined || siteid.trim() == "") {
    ...
    let start = new Date(parseInt(startDate.value), 0, 1, 7, 0, 0);
    let end = new Date(parseInt(startDate.value), 0, 1, 7, 0, 0);
    ...
    let url = `${urlGetQuantityMonthForcast}/${siteid}/${totalMilisecondStart}/${totalMilisecondEnd}`;

    getData(url);
    titleChart.innerHTML = `Biểu đồ sản lượng và dự báo sản lượng hàng tháng của năm ${start.getFullYear()}`;
    titlChartCircle.innerHTML = `Biểu đồ tổng sản lượng thật và tổng dự báo sản lượng hàng tháng của năm ${start.getFullYear()}`;
  }
});
...
function createTable(data) {
  ...
  $("#example").DataTable({
    ...
    buttons: [
      {
        extend: "excelHtml5",
        title: `Bao_Cao_San_Luong_Va_Du_Bao_San_Luong_Hang_Thang_Theo_Nam`,
      },
      {
        extend: "csvHtml5",
        title: `Bao_Cao_San_Luong_Va_Du_Bao_San_Luong_Hang_Thang_Theo_Nam`,
