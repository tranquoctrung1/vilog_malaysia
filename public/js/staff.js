...
function GetStaff() {
    axios
        ...
        .then((res) => {
            ...
            new TomSelect('#modalFullNameStaff', {
            ...
            createOptionsInStaffAndConsumerSelectBox(res.data, 'staffId');
            staffSelect = new TomSelect('#staffId', {
    ...
    axios
        ...
        .then((res) => {
            if (res.data.length > 0) {
                modalFullNameStaff.value = fillDataIntoInputTag(
                    res.data[0].FullName,
                );
                modalTelephoneStaff.value = fillDataIntoInputTag(
                    res.data[0].Telephone,
                );
                modalAddressStaff.value = fillDataIntoInputTag(
                    res.data[0].Adrress,
                );
...
function insertStaff(e) {
    if (
        ...
        let url = `${urlInsertStaff}/${CreateDataNullForPost(
            ...
            modalAddressStaff.value.replaceAll('/', '_'),
        )}`;
...
function deleteStaff(e) {
    if (
        ...
        let url = `${urlDeleteStaff}/${CreateDataNullForPost(
            modalIdStaff.value,
        )}`;
