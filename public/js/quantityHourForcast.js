...
let urlGetQuantityHourForcast = `${hostname}/GetQuantityHourForcast`;
...
let viewQuantityHourForcast = document.getElementById(
  "viewQuantityHourForcast"
);

viewQuantityHourForcast.addEventListener("click", function (e) {
  ...
  if (siteid == null || siteid == undefined || siteid.trim() == "") {
    ...
    let url = `${urlGetQuantityHourForcast}/${siteid}/${totalMilisecondStart}/${totalMilisecondEnd}`;

    getData(url);
    titleChart.innerHTML = `Biểu đồ sản lượng và dự báo sản lượng hàng giờ của ngày ${start.getDate()}`;
    titlChartCircle.innerHTML = `Biểu đồ tổng sản lượng thật và tổng dự báo sản lượng hàng giờ của ngày ${start.getDate()}`;
  }
});
...
function createTable(data) {
  ...
  $("#example").DataTable({
    ...
    buttons: [
      {
        extend: "excelHtml5",
        title: `Bao_Cao_San_Luong_Va_Du_Bao_San_Luong_Hang_Gio_Theo_Ngay`,
      },
      {
        extend: "csvHtml5",
        title: `Bao_Cao_San_Luong_Va_Du_Bao_San_Luong_Hang_Gio_Theo_Ngay`,
        ...
        if (res.data != null && res.data != undefined) {
          if (res.data[0] != null && res.data[0] != undefined) {
            if (
              ...
              startDate.value = convertDateToDateInputTag(
                new Date(res.data[0].TimeStamp)
              );
              ...
              let urlGetData = `${urlGetQuantityHourForcast}/${siteid.value}/${totalMilisecondStart}/${totalMilisecondEnd}`;
